...
// Do not edit! See https://mongoose.ws/docs/guides/web-ui-builder/#custom-api-handlers
static struct coils s_coils[] = {
  {false},
  {false},
  {false},
};
bool glue_get_coils(struct coils *data, size_t i) {
  size_t array_size = sizeof(s_coils) / sizeof(s_coils[0]);
  if (i >= array_size) return false;
  *data = s_coils[i];  // Sync with your device
  return true;
}
void glue_set_coils(struct coils *data, size_t i) {
  size_t array_size = sizeof(s_coils) / sizeof(s_coils[0]);
  if (i < array_size) s_coils[i] = *data; // Sync with your device
}