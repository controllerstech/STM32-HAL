...
bool glue_get_inputs(struct inputs *data, size_t i) {
  size_t array_size = sizeof(s_inputs) / sizeof(s_inputs[0]);
  if (i >= array_size) return false;
  *data = s_inputs[i];  // Sync with your device
  return true;
}
void glue_set_inputs(struct inputs *data, size_t i) {
  size_t array_size = sizeof(s_inputs) / sizeof(s_inputs[0]);
  if (i < array_size) s_inputs[i] = *data; // Sync with your device
}